...
export default function ExcelViewer({ url, fileName, className }: Props) {
    ...
    React.useEffect(() => {
        ...
        const check = async () => {
            ...
            try {
                ...
                if (alive) setError(e instanceof Error ? e.message : "Failed to load Excel file")
            } finally {
    ...
    return (
        <div className={"w-full max-w-full" + (className ? ` ${className}` : "")}>
            <div className="flex items-center justify-between gap-2 mb-2">
                <span className="text-xs" aria-label="Excel name">{fileName || "Excel"}</span>
                <a
                    href={url}
                    target="_blank"
                    rel="noreferrer"
                    className="text-xs underline"
                    aria-label="Open Excel in new tab"
                >Open</a>
            </div>
