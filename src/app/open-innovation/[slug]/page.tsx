...
export function generateStaticParams() {
  return articles
    .filter((article) => article.category === "open-innovation")
    .map((article) => ({ slug: article.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const article = getArticle("open-innovation", slug);
...
export default async function OpenInnovationArticlePage({ params }: PageProps) {
  const { slug } = await params;
  const article = getArticle("open-innovation", slug);
