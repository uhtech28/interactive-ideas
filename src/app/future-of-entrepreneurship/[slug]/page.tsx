...
export function generateStaticParams() {
  return articles
    .filter((article) => article.category === "future-of-entrepreneurship")
    .map((article) => ({ slug: article.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const article = getArticle("future-of-entrepreneurship", slug);
...
export default async function FutureOfEntrepreneurshipArticlePage({
  params,
}: PageProps) {
  const { slug } = await params;
  const article = getArticle("future-of-entrepreneurship", slug);
