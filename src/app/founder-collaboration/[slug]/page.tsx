...
export function generateStaticParams() {
  return articles
    .filter((article) => article.category === "founder-collaboration")
    .map((article) => ({ slug: article.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const article = getArticle("founder-collaboration", slug);
...
export default async function FounderCollaborationArticlePage({ params }: PageProps) {
  const { slug } = await params;
  const article = getArticle("founder-collaboration", slug);
